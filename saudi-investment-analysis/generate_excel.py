...
# ─── GLOSSARY TERM LISTS ──────────────────────────────────────────────────────
GLOSS_OVERVIEW = [
    ("EPS — ربح السهم", "صافي الربح ÷ عدد الأسهم  |  الصيغة: =ربح 2024 (م.) ÷ الأسهم (م.)  |  النتيجة: ريال/سهم"),
    ("P/E — مكرر الربحية", "السعر ÷ EPS  |  يقيس كم يدفع المستثمر مقابل كل ريال ربح  |  أقل = أرخص نسبياً"),
    ("P/B — مكرر القيمة الدفترية", "(السعر × الأسهم) ÷ حقوق الملكية  |  نسبة < 1 تعني تداول السهم بأقل من قيمته الدفترية"),
    ("ROE% — العائد على حقوق الملكية", "صافي الربح ÷ حقوق الملكية × 100  |  يقيس كفاءة توظيف أموال المساهمين  |  كلما ارتفع كان أفضل"),
    ("DPS — توزيع السهم", "إجمالي التوزيعات النقدية المُعلنة خلال العام ÷ عدد الأسهم  |  ريال/سهم"),
    ("عائد التوزيع%", "DPS ÷ السعر × 100  |  يمثّل الدخل النقدي السنوي كنسبة من قيمة الاستثمار"),
    ("نسبة التوزيع%", "DPS ÷ EPS × 100  |  ما يُعاد للمساهمين من الأرباح  |  أقل من 80% يُعدّ مستداماً"),
    ("م. = مليون ريال", "وحدة القياس المستخدمة لجميع الأرقام المالية في هذا الجدول"),
    ("🟡 Blend", "شركة تجمع خصائص النمو والقيمة — تتمتع بأرباح مستقرة مع إمكانية توسع معقولة"),
    ("🔵 Growth", "شركة ذات نمو مرتفع في الأرباح والإيرادات — تُقيَّم عادةً بمكررات أعلى من السوق"),
    ("🟢 Value", "شركة تُتداول بأقل من قيمتها الجوهرية — تمثّل فرصة للمستثمر الباحث عن هامش أمان"),
    ("تراكم بقوة", "توصية بشراء قوي — يُنصح ببناء مركز مرتفع (> 10% من المحفظة)"),
    ("تراكم", "توصية بالشراء التدريجي وبناء مركز في السهم"),
    ("احتفاظ / تراكم عند الضعف", "الإبقاء على الحصة الحالية مع الشراء عند أي تراجع في السعر"),
    ("مراقبة", "متابعة السهم عن كثب دون التصرف حتى تتضح صورة الأداء القادم"),
]

GLOSS_DIVIDENDS = [
    ("DPS — توزيع السهم", "مبلغ التوزيع النقدي لكل سهم واحد بالريال السعودي"),
    ("نسبة التوزيع%", "إجمالي التوزيع (م.) ÷ صافي الربح × 100  |  أقل من 80% مستدام  |  🟢 <60%  🟡 60-80%  🔴 >80%"),
    ("H1 / H2", "النصف الأول / النصف الثاني من السنة"),
    ("Q1 / Q2 / Q3 / Q4", "الربع الأول / الثاني / الثالث / الرابع من السنة"),
    ("م. = مليون ريال", "وحدة قياس إجمالي التوزيعات وصافي الربح"),
    ("⚠ قبل المنح", "تُطلب مراجعة هذا المبلغ بعد إصدار أسهم مجانية لتعديله بنسبة المنح"),
    ("ملخص سنوي", "إجمالي التوزيعات النقدية للسهم الواحد خلال السنة الكاملة (جمع كل الفترات)"),
]

GLOSS_FINANCIALS = [
    ("CAGR 3Y — معدل النمو السنوي المركب", "الصيغة: (قيمة 2024 ÷ قيمة 2021)^(1/3) − 1  |  يقيس متوسط النمو السنوي على 3 سنوات"),
    ("نمو 2024/2023 — YoY", "الصيغة: (2024 − 2023) ÷ |2023|  |  التغيير السنوي المقارن"),
    ("الإيرادات", "إجمالي الدخل التشغيلي (المبيعات / العائد على التمويل للبنوك)  |  م. ريال"),
    ("صافي الربح", "الأرباح بعد خصم جميع التكاليف والضرائب  |  م. ريال"),
    ("OCF — التدفق النقدي التشغيلي", "النقد الفعلي المتولد من العمليات (أقل تأثراً بالسياسات المحاسبية من الربح)  |  م. ريال"),
    ("حقوق المساهمين", "إجمالي الأصول − إجمالي الالتزامات  |  القيمة الدفترية للشركة  |  م. ريال"),
    ("م. = مليون ريال", "وحدة قياس جميع الأرقام في هذه الورقة"),
]

GLOSS_VALUATION = [
    ("P/E — مكرر الربحية", "السعر ÷ EPS  |  القيمة العادلة = متوسط P/E القطاعي × EPS 2024"),
    ("P/B — مكرر القيمة الدفترية", "(سعر × أسهم) ÷ حقوق الملكية  |  القيمة العادلة = متوسط P/B القطاعي × القيمة الدفترية للسهم"),
    ("DDM — نموذج خصم التوزيعات", "DPS₁ ÷ (WACC − g)  |  Gordon Growth Model  |  مناسب لشركات التوزيع المنتظم"),
    ("DCF — التدفقات النقدية المخصومة", "Σ FCF÷(1+WACC)^t + Terminal Value  |  5 سنوات + قيمة نهائية  |  g_terminal = 3%"),
    ("EV/EBITDA", "قيمة المنشأة ÷ EBITDA  |  مضاعف القطاع × OCF (كـProxy للـEBITDA) ÷ الأسهم"),
    ("PEG", "P/E ÷ معدل نمو EPS%  |  PEG < 1 يشير لتقييم مغرٍ نسبياً  |  لا ينطبق عند الخسارة"),
    ("P/S — السعر إلى الإيرادات", "(سعر × أسهم) ÷ الإيرادات  |  مفيد لتقييم شركات النمو أو عند غياب الربحية"),
    ("هامش الأمان%", "(متوسط القيمة العادلة − السعر) ÷ السعر × 100  |  🟢 موجب = هامش أمان  |  🔴 سالب = علاوة سعرية"),
    ("WACC — تكلفة رأس المال", "متوسط تكلفة رأس المال المرجح  |  معدل الخصم في نموذج DCF  |  يتفاوت بين 8.5% - 13% حسب القطاع"),
    ("إجماع المحللين", "متوسط أسعار مستهدفة من بيوت خبرة متعددة  |  للتفاصيل: راجع ورقة بيوت الخبرة"),
]

GLOSS_ANALYST = [
    ("السعر المستهدف", "تقدير المحلل للقيمة العادلة للسهم خلال 12 شهراً القادمة  |  بالريال السعودي"),
    ("Upside% — الارتفاع المتوقع", "(السعر المستهدف − السعر الحالي) ÷ السعر الحالي × 100  |  🟢 موجب = فرصة  |  🔴 سالب = مبالغ في قيمته"),
    ("OW — Overweight (زيادة المراكز)", "توصية بوزن أعلى من المعيار المرجعي — تعادل Buy/Strong Buy في التصنيفات الأخرى"),
    ("Neutral — محايد", "الإبقاء على المراكز الحالية دون زيادة أو تخفيض  |  تعادل Hold"),
    ("إجماع (Consensus)", "متوسط أسعار مستهدفة ومتوسط التوصيات من عدة بيوت خبرة"),
    ("مصدر البيانات", "Investing.com / Bloomberg / الجزيرة كابيتال / مورغان ستانلي  |  مايو 2026"),
]

GLOSS_PORTFOLIO = [
    ("الوزن المقترح%", "نسبة تخصيص الاستثمار في كل ورقة مالية من إجمالي قيمة المحفظة"),
    ("مبلغ مقترح (100K)", "التوزيع بالريال لمحفظة افتراضية قيمتها 100,000 ريال  |  يُعدَّل تناسبياً لأي قيمة أخرى"),
    ("🟢 Value", "شركة قيمة — تُتداول بأقل من قيمتها الجوهرية — تناسب المستثمر المحافظ"),
    ("🔵 Growth", "شركة نمو — توسع مرتفع وأرباح متصاعدة — تناسب المستثمر طويل المدى"),
    ("🟡 Blend", "مزيج قيمة ونمو — توازن بين الاستقرار والنمو"),
    ("كاش", "سيولة احتياطية لاستغلال فرص الشراء عند تراجعات السوق"),
    ("مستوى المخاطرة", "تقدير نوعي: منخفض / منخفض-متوسط / متوسط / متوسط-مرتفع / مرتفع"),
]
...
# ─── GLOSSARY HELPER ──────────────────────────────────────────────────────────
def write_glossary(ws, start_row, terms, total_cols):
    """Writes a compact glossary/legend table at the bottom of a worksheet.
    terms: list of (term_str, explanation_str) tuples
    """
    # Blank separator row
    ...
    c = ws[f"A{start_row}"]
    c.value = "📖  مسرد المصطلحات والاختصارات المستخدمة في هذه الورقة"
    ...
    # Sub-headers
    term_end = min(4, total_cols - 1)
    exp_start = term_end + 1
    ws.merge_cells(f"A{start_row}:{get_column_letter(term_end)}{start_row}")
    c = ws.cell(row=start_row, column=1, value="المصطلح / الاختصار")
    c.font = hdr_font(9)
    c.fill = fill("2E5090")
    c.alignment = center()
    c.border = make_border()
    ws.merge_cells(f"{get_column_letter(exp_start)}{start_row}:{get_column_letter(total_cols)}{start_row}")
    c = ws.cell(row=start_row, column=exp_start, value="الشرح والصيغة")
    c.font = hdr_font(9)
    c.fill = fill("2E5090")
    c.alignment = center()
    c.border = make_border()
    ...
    for i, (term, explanation) in enumerate(terms):
        row_bg = "EEF4FF" if i % 2 == 0 else "F8FAFF"
        ws.merge_cells(f"A{start_row}:{get_column_letter(term_end)}{start_row}")
        c = ws.cell(row=start_row, column=1, value=term)
        c.font = Font(name='Calibri', size=9, bold=True, color="1A3660")
        c.fill = fill("DCE8F8" if i % 2 == 0 else "E8F0FD")
        c.alignment = Alignment(horizontal='right', vertical='center', readingOrder=2)
        c.border = make_border("C8D8F0")
        ws.merge_cells(f"{get_column_letter(exp_start)}{start_row}:{get_column_letter(total_cols)}{start_row}")
        c = ws.cell(row=start_row, column=exp_start, value=explanation)
